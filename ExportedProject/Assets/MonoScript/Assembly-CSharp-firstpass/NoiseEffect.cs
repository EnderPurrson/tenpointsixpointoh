...
public class NoiseEffect : MonoBehaviour
{
	...
	private void OnRenderImage(RenderTexture source, RenderTexture destination)
	{
		...
		float single = 1f / this.grainSize;
		material.SetVector("_GrainOffsetScale", new Vector4(UnityEngine.Random.@value, UnityEngine.Random.@value, (float)Screen.width / (float)this.grainTexture.width * single, (float)Screen.height / (float)this.grainTexture.height * single));

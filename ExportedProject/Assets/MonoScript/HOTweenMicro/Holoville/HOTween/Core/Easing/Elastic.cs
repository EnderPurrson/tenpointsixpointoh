...
namespace Holoville.HOTween.Core.Easing
{
	public static class Elastic
	{
		...
		public static float EaseIn(float time, float startValue, float changeValue, float duration, float amplitude, float period)
		{
			...
			{
				num = period / ((float)Math.PI * 2f) * (float)Math.Asin((double)(changeValue / amplitude));
			}
			return 0f - amplitude * (float)Math.Pow(2.0, (double)(10f * (time -= 1f))) * (float)Math.Sin((double)((time * duration - num) * ((float)Math.PI * 2f) / period)) + startValue;
		}
		...
		public static float EaseOut(float time, float startValue, float changeValue, float duration, float amplitude, float period)
		{
			...
			return amplitude * (float)Math.Pow(2.0, (double)(-10f * time)) * (float)Math.Sin((double)((time * duration - num) * ((float)Math.PI * 2f) / period)) + changeValue + startValue;
		}
		...
		public static float EaseInOut(float time, float startValue, float changeValue, float duration, float amplitude, float period)
		{
			...
			if (time < 1f)
			{
				return -0.5f * (amplitude * (float)Math.Pow(2.0, (double)(10f * (time -= 1f))) * (float)Math.Sin((double)((time * duration - num) * ((float)Math.PI * 2f) / period))) + startValue;
			}
			return amplitude * (float)Math.Pow(2.0, (double)(-10f * (time -= 1f))) * (float)Math.Sin((double)((time * duration - num) * ((float)Math.PI * 2f) / period)) * 0.5f + changeValue + startValue;
		}
	}
}

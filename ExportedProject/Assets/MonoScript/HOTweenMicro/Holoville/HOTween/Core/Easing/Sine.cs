...
namespace Holoville.HOTween.Core.Easing
{
	public static class Sine
	{
		...
		{
			return (0f - changeValue) * (float)Math.Cos((double)(time / duration * ((float)Math.PI / 2f))) + changeValue + startValue;
		}
		...
		{
			return changeValue * (float)Math.Sin((double)(time / duration * ((float)Math.PI / 2f))) + startValue;
		}
		...
		{
			return (0f - changeValue) * 0.5f * ((float)Math.Cos((double)((float)Math.PI * time / duration)) - 1f) + startValue;
		}
	}
}

...
namespace Holoville.HOTween
{
	public class HOTween : MonoBehaviour
	{
		...
		internal static bool isUpdateLoop
		{
			...
			get
			{
				return _003CisUpdateLoop_003Ek__BackingField;
			}
			[CompilerGenerated]
			private set
			{
				_003CisUpdateLoop_003Ek__BackingField = value;
			}
		}
		...
		private void OnDrawGizmos()
		{
			...
			{
				PlugVector3Path plugVector3Path = plugins.get_Item(i) as PlugVector3Path;
		...
		private static global::System.Collections.IEnumerator TimeScaleIndependentUpdate()
		{
			while (tweens != null)
			{
		...
		internal static void RemoveFromTweens(IHOTweenComponent p_tween)
		{
			...
			int count = tweens.get_Count();
			for (int i = 0; i < count; i++)
			{
				if (tweens.get_Item(i) == p_tween)
				{
					tweens.RemoveAt(i);
					break;
				}
		...
		{
			List<IHOTweenComponent> val = new List<IHOTweenComponent>();
			if (tweens == null)
			{
				return val;
			}
			...
			for (int i = 0; i < count; i++)
			{
				ABSTweenComponent aBSTweenComponent = tweens.get_Item(i);
				if (p_includeNestedTweens)
				{
					val.AddRange((global::System.Collections.Generic.IEnumerable<IHOTweenComponent>)aBSTweenComponent.GetTweensById(p_id));
				}
				else if (aBSTweenComponent.id == p_id)
				{
					val.Add((IHOTweenComponent)aBSTweenComponent);
				}
			}
			return val;
		}
		...
		public static List<IHOTweenComponent> GetTweensByIntId(int p_intId, bool p_includeNestedTweens)
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				if (p_includeNestedTweens)
				{
					val.AddRange((global::System.Collections.Generic.IEnumerable<IHOTweenComponent>)aBSTweenComponent.GetTweensByIntId(p_intId));
				}
		...
		{
			List<Tweener> val = new List<Tweener>();
			if (tweens == null)
			{
				return val;
			}
			...
			for (int i = 0; i < count; i++)
			{
				...
				{
					val.AddRange((global::System.Collections.Generic.IEnumerable<Tweener>)((Sequence)aBSTweenComponent).GetTweenersByTarget(p_target));
				}
			}
			return val;
		}
		...
		public static bool IsTweening(object p_target)
		{
			...
			for (int i = 0; i < count; i++)
			{
				if (tweens.get_Item(i).IsTweening(p_target))
				{
					return true;
				}
			}
		...
		public static bool IsTweening(string p_id)
		{
			...
			for (int i = 0; i < count; i++)
			{
				if (tweens.get_Item(i).IsTweening(p_id))
				{
					return true;
				}
			}
		...
		public static TweenInfo[] GetTweenInfos()
		{
			...
			for (int i = 0; i < count; i++)
			{
				array[i] = new TweenInfo(tweens.get_Item(i));
			}
			return array;
		}
		...
		private static void DoUpdate(UpdateType p_updateType, float p_elapsed)
		{
			...
			isUpdateLoop = false;
			int count2 = tweensToRemoveIndexes.get_Count();
			if (count2 > 0)
			{
				tweensToRemoveIndexes.Sort();
				for (int j = 0; j < count2; j++)
				{
					tweens.RemoveAt(tweensToRemoveIndexes.get_Item(j) - j);
				}
			}
			int count3 = onCompletes.get_Count();
			if (count3 > 0)
			{
				for (int k = 0; k < count3; k++)
				{
					onCompletes.get_Item(k).OnCompleteDispatch();
				}
		...
		{
			tweens.get_Item(p_index).Kill(false);
			if (isUpdateLoop)
			{
		...
		{
			tweens.get_Item(p_index).Pause();
		}

		private static void DoFilteredPlay(int p_index, bool p_skipDelay)
		{
			ABSTweenComponent aBSTweenComponent = tweens.get_Item(p_index);
			Tweener tweener = aBSTweenComponent as Tweener;
			if (tweener != null)
			{
		...
		{
			tweens.get_Item(p_index).Reverse(p_forcePlay);
		}

		private static void DoFilteredComplete(int p_index, bool p_optionalBool)
		{
			tweens.get_Item(p_index).Complete(false);
		}
		...
		private global::System.Collections.IEnumerator StartCoroutines_StartTimeScaleIndependentUpdate()
		{
			yield return null;
		...
		private static void SetGOName()
		{
			...
			{
				tweenGOInstance.name = string.Concat((object)"HOTween : ", (object)totTweens);
			}
		}

		private static bool CheckClear()
		{
			if (tweens == null || tweens.get_Count() == 0)
			{
				Clear();
				if (isPermanent)
				{
		...
		private static int DoFilteredIteration(object p_filter, TweenDelegate.FilterFunc p_operation, bool p_collectionChanger, bool p_optionalBool)
		{
			...
			else if (p_filter is int)
			{
				...
				{
					if (tweens.get_Item(num5).intId == num4)
					{
			...
			else if (p_filter is string)
			{
				...
				{
					if (tweens.get_Item(num6).id == text)
					{
			...
			else if (p_filter is Tweener)
			{
				...
				{
					if (tweens.get_Item(num7) == tweener)
					{
			...
			else if (p_filter is Sequence)
			{
				...
				{
					if (tweens.get_Item(num8) == sequence)
					{
			...
			else
			{
				...
				{
					Tweener tweener2 = tweens.get_Item(num9) as Tweener;
		...
		private static List<ABSTweenPlugin> GetPlugins()
		{
			...
			List<ABSTweenPlugin> val = new List<ABSTweenPlugin>();
			int count = tweens.get_Count();
			for (int i = 0; i < count; i++)
			{
				tweens.get_Item(i).FillPluginsList(val);
			}
			return val;
		}
	}

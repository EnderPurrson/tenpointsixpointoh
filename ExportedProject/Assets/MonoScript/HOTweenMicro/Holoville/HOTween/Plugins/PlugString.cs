...
namespace Holoville.HOTween.Plugins
{
	...
	{
		internal static global::System.Type[] validPropTypes = new global::System.Type[1] { typeof(string) };

		internal static global::System.Type[] validValueTypes = new global::System.Type[1] { typeof(string) };
		...
		{
			changeVal = typedEndVal.get_Length();
		}
		...
		protected override void SetIncremental(int p_diffIncr)
		{
			...
			{
				typedStartVal = typedStartVal.Substring(0, typedStartVal.get_Length() + typedEndVal.get_Length() * p_diffIncr);
			}
		}

		protected override void DoUpdate(float p_totElapsed)
		{
			int num = (int)Math.Round((double)ease(p_totElapsed, 0f, changeVal, _duration, tweenObj.easeOvershootOrAmplitude, tweenObj.easePeriod));
			string value = ((!isRelative) ? (typedEndVal.Substring(0, num) + (((float)num >= changeVal || num >= typedStartVal.get_Length()) ? "" : typedStartVal.Substring(num))) : (typedStartVal + typedEndVal.Substring(0, num)));
			SetValue(value);
		}
	}
}

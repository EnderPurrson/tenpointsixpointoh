	...
	public class Sequence : ABSTweenComponent
	{
		...
		internal override bool steadyIgnoreCallbacks
		{
			...
			set
			{
				...
				{
					HOTSeqItem hOTSeqItem = items.get_Item(i);
		...
		{
			hasCallbacks = true;
			HOTSeqItem hOTSeqItem = new HOTSeqItem(p_time, p_callback, p_callbackWParms, p_callbackParms);
			if (items == null)
			{
				List<HOTSeqItem> val = new List<HOTSeqItem>();
				val.Add(hOTSeqItem);
				items = val;
			}
			else
			{
				...
				for (int i = 0; i < count; i++)
				{
					if (items.get_Item(i).startTime >= p_time)
					{
		...
		private float Prepend(IHOTweenComponent p_twMember, float p_duration)
		{
			...
			for (int i = 0; i < count; i++)
			{
				items.get_Item(i).startTime += num;
			}
		...
		public override List<object> GetTweenTargets()
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				if (hOTSeqItem.seqItemType == SeqItemType.Tween)
				{
					val.AddRange((global::System.Collections.Generic.IEnumerable<object>)hOTSeqItem.twMember.GetTweenTargets());
				}
			}
			return val;
		}
		...
		public List<Tweener> GetTweenersByTarget(object p_target)
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				{
					val.AddRange((global::System.Collections.Generic.IEnumerable<Tweener>)((Sequence)hOTSeqItem.twMember).GetTweenersByTarget(p_target));
				}
			}
			return val;
		}
		...
		internal override List<IHOTweenComponent> GetTweensById(string p_id)
		{
			...
			if (base.id == p_id)
			{
				val.Add((IHOTweenComponent)this);
			}
			int count = items.get_Count();
			for (int i = 0; i < count; i++)
			{
				...
				if (hOTSeqItem.seqItemType == SeqItemType.Tween)
				{
					val.AddRange((global::System.Collections.Generic.IEnumerable<IHOTweenComponent>)hOTSeqItem.twMember.GetTweensById(p_id));
				}
			}
			return val;
		}
		...
		internal override List<IHOTweenComponent> GetTweensByIntId(int p_intId)
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				if (hOTSeqItem.seqItemType == SeqItemType.Tween)
				{
					val.AddRange((global::System.Collections.Generic.IEnumerable<IHOTweenComponent>)hOTSeqItem.twMember.GetTweensByIntId(p_intId));
				}
			}
			return val;
		}
		...
		internal override bool Update(float p_shortElapsed, bool p_forceUpdate, bool p_isStartupIteration, bool p_ignoreCallbacks)
		{
			...
			if (hasCallbacks && !_isPaused)
			{
				...
				for (int i = 0; i < count; i++)
				{
					...
					if ((!_isLoopingBack) ? ((!flag5 && (hOTSeqItem.startTime <= num3 || _completedLoops != prevCompletedLoops) && hOTSeqItem.startTime >= num4) || (hOTSeqItem.startTime <= num3 && ((!_isComplete && _completedLoops != prevCompletedLoops) || hOTSeqItem.startTime >= num4))) : ((flag5 && (hOTSeqItem.startTime >= num3 || _completedLoops != prevCompletedLoops) && hOTSeqItem.startTime <= num4) || (hOTSeqItem.startTime >= num3 && ((!_isComplete && _completedLoops != prevCompletedLoops) || hOTSeqItem.startTime <= num4))))
					{
						if (val == null)
						{
							val = new List<HOTSeqItem>();
						}
						if (hOTSeqItem.startTime > num3)
						{
							val.Insert(0, hOTSeqItem);
						}
						else
						{
							val.Add(hOTSeqItem);
				...
				if (val != null)
				{
					Enumerator<HOTSeqItem> enumerator = val.GetEnumerator();
					try
					{
						while (enumerator.MoveNext())
						{
							HOTSeqItem current = enumerator.get_Current();
							if (current.callback != null)
							{
								current.callback();
							}
							else if (current.callbackWParms != null)
							{
								current.callbackWParms(new TweenEvent(this, current.callbackParms));
							}
						}
					}
					finally
			...
			if (_duration > 0f)
			{
				...
				{
					HOTSeqItem hOTSeqItem = items.get_Item(num6);
				...
				{
					HOTSeqItem hOTSeqItem = items.get_Item(j);
					if (hOTSeqItem.seqItemType == SeqItemType.Tween && hOTSeqItem.startTime <= num5)
					{
						if (hOTSeqItem.twMember.duration > 0f)
						{
			...
			else
			{
				...
				{
					HOTSeqItem hOTSeqItem = items.get_Item(num7);
					if (hOTSeqItem.seqItemType == SeqItemType.Tween)
					{
						hOTSeqItem.twMember.Complete();
					}
		...
		private void Rewind(bool p_play)
		{
			...
			int num = items.get_Count() - 1;
			for (int num2 = num; num2 > -1; num2--)
			{
				HOTSeqItem hOTSeqItem = items.get_Item(num2);
				if (hOTSeqItem.seqItemType == SeqItemType.Tween)
				{
					hOTSeqItem.twMember.Rewind();
				}
		...
		private void TweenStartupIteration()
		{
			...
			{
				HOTSeqItem hOTSeqItem = items.get_Item(num);
				if (hOTSeqItem.seqItemType == SeqItemType.Tween)
				{
					hOTSeqItem.twMember.Rewind();
				}

...
namespace Holoville.HOTween
{
	public class TweenParms : ABSTweenComponentParms
	{
		...
		private static readonly Dictionary<global::System.Type, string> _TypeToShortString;
		...
		internal void InitializeObject(Tweener p_tweenObj, object p_target)
		{
			...
			global::System.Type type = p_target.GetType();
			FieldInfo val = null;
			int count = propDatas.get_Count();
			for (int i = 0; i < count; i++)
			{
				HOTPropData hOTPropData = propDatas.get_Item(i);
				PropertyInfo property = type.GetProperty(hOTPropData.propName);
				if (property == null)
				{
					val = type.GetField(hOTPropData.propName);
					if (val == null)
					{
						TweenWarning.Log(string.Concat(new object[5] { "\"", p_target, ".", hOTPropData.propName, "\" is missing, static, or not public. The tween for this property will not be created." }));
						continue;
					}
				}
				...
				if (aBSTweenPlugin != null)
				{
					...
					{
						TweenWarning.Log(string.Concat(new object[4]
						{
							Utils.SimpleClassName(((object)aBSTweenPlugin2).GetType()),
							" : Invalid target (",
							p_target,
				...
				else
				{
					...
					switch ((property == null) ? (_TypeToShortString.ContainsKey(val.get_FieldType()) ? _TypeToShortString.get_Item(val.get_FieldType()) : "") : (_TypeToShortString.ContainsKey(property.get_PropertyType()) ? _TypeToShortString.get_Item(property.get_PropertyType()) : ""))
					{
					case "Vector2":
					...
					default:
						...
						{
							TweenWarning.Log(string.Concat(new object[7]
							{
								"No valid plugin for animating \"",
								p_target,
								".",
								hOTPropData.propName,
								"\" (of type ",
								(property != null) ? property.get_PropertyType() : val.get_FieldType(),
								"). The tween for this property will not be created."
							}));
							continue;
						}
					...
					{
						TweenWarning.Log(string.Concat(new object[5] { "The end value set for \"", p_target, ".", hOTPropData.propName, "\" tween is invalid. The tween for this property will not be created." }));
						continue;
					}
				}
				aBSTweenPlugin2.Init(p_tweenObj, hOTPropData.propName, easeType, type, property, val);
		...
		private static bool ValidateValue(object p_val, global::System.Type[] p_validVals)
		{
			return global::System.Array.IndexOf<global::System.Type>(p_validVals, p_val.GetType()) != -1;
		}

		static TweenParms()
		{
			Dictionary<global::System.Type, string> val = new Dictionary<global::System.Type, string>(8);
			val.Add(typeof(Vector2), "Vector2");
			val.Add(typeof(Vector3), "Vector3");
			val.Add(typeof(Vector4), "Vector4");
			val.Add(typeof(Quaternion), "Quaternion");
			val.Add(typeof(Color), "Color");
			val.Add(typeof(Color32), "Color32");
			val.Add(typeof(Rect), "Rect");
			val.Add(typeof(string), "String");
			val.Add(typeof(int), "Int32");
			_TypeToShortString = val;
		}
	}
}

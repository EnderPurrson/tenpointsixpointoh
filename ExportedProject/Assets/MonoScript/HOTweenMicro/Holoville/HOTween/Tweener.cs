...
namespace Holoville.HOTween
{
	public class Tweener : ABSTweenComponent
	{
		...
		public bool isFrom
		{
			...
			get
			{
				return _003CisFrom_003Ek__BackingField;
			}
			[CompilerGenerated]
			internal set
			{
				_003CisFrom_003Ek__BackingField = value;
			}
		}
		...
		public EaseType easeType
		{
			...
			set
			{
				...
				{
					plugins.get_Item(i).SetEase(_easeType);
				}
			}
		}
		...
		internal Tweener(object p_target, float p_duration, TweenParms p_parms)
		{
			...
			if (plugins != null && plugins.get_Count() > 0)
			{
				_isEmpty = false;
			}
			SetFullDuration();
		...
		protected override void Reset()
		{
			...
			isPartialled = false;
			pv3Path = null;
			_delay = (_elapsedDelay = (delayCount = 0f));
			_pixelPerfect = false;
			_speedBased = false;
		...
		{
			if (plugins.get_Count() > 1)
			{
				TweenWarning.Log(string.Concat((object)"Applying a partial path on a Tweener (", _target, (object)") with more than one plugin/property being tweened is not allowed"));
				return this;
			}
			if (pv3Path == null)
			{
				...
				if (pv3Path == null)
				{
					TweenWarning.Log(string.Concat((object)"Tweener for ", _target, (object)" contains no PlugVector3Path plugin"));
					return this;
				}
			}
			Startup();
		...
		private PlugVector3Path GetPlugVector3PathPlugin()
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				PlugVector3Path plugVector3Path = aBSTweenPlugin as PlugVector3Path;
		...
		internal bool Update(float p_shortElapsed, bool p_forceUpdate, bool p_isStartupIteration, bool p_ignoreCallbacks, bool p_ignoreDelay = false)
		{
			...
			if (_target == null || _target.Equals((object)default(object)))
			{
				Kill(false);
				return true;
			}
		...
		internal override void SetIncremental(int p_diffIncr)
		{
			if (plugins != null)
			{
				...
				for (int i = 0; i < count; i++)
				{
					plugins.get_Item(i).ForceSetIncremental(p_diffIncr);
				}
			}
		}
		...
		internal void ForceSetSpeedBasedDuration()
		{
			...
			for (int i = 0; i < count; i++)
			{
				plugins.get_Item(i).ForceSetSpeedBasedDuration();
			}
			_duration = 0f;
			for (int j = 0; j < count; j++)
			{
				ABSTweenPlugin aBSTweenPlugin = plugins.get_Item(j);
		...
		private void Startup(bool p_force)
		{
			...
			if (_speedBased)
			{
				...
				for (int j = 0; j < count; j++)
				{
					ABSTweenPlugin aBSTweenPlugin2 = plugins.get_Item(j);
		...
		internal override void FillPluginsList(List<ABSTweenPlugin> p_plugs)
		{
			if (plugins != null)
			{
				...
				for (int i = 0; i < count; i++)
				{
					p_plugs.Add(plugins.get_Item(i));
				}
			}
		}

...
namespace Prime31
{
	public class MonoBehaviourGUI : MonoBehaviour
	{
		...
		private void paintWindow(int id)
		{
			...
			{
				_logBuilder.Remove(0, _logBuilder.get_Length());
			}
			GUILayout.Label(((object)_logBuilder).ToString());
		...
		private void Update()
		{
			if (!_logRegistered)
			{
				...
				_logRegistered = true;
				_isWindowsPhone = ((object)Application.platform).ToString().ToLower().Contains("wp8");
			}
			bool flag = false;
		...
		protected bool toggleButton(string defaultText, string selectedText)
		{
			...
			{
				_toggleButtons.set_Item(defaultText, true);
			}
			string text = ((!_toggleButtons.get_Item(defaultText)) ? selectedText : defaultText);
			GUI.skin.button.normal.background = toggleButtonBackground;
			if (!_toggleButtons.get_Item(defaultText))
			{
			...
			{
				_toggleButtons.set_Item(defaultText, text != defaultText);
			}
			GUI.skin.button.normal.background = normalBackground;
			GUI.skin.button.fontStyle = FontStyle.Normal;
			GUI.contentColor = Color.white;
			return _toggleButtons.get_Item(defaultText);
		}
		...
		public static void loadLevel(int level)
		{
			try
			{
				((MethodBase)typeof(Application).GetMethod("LoadLevel", new global::System.Type[1] { typeof(int) })).Invoke((object)default(object), new object[1] { level });
			}
			catch
			{
				try
		...
		public static void loadLevel(string level)
		{
			try
			{
				((MethodBase)typeof(Application).GetMethod("LoadLevel", new global::System.Type[1] { typeof(string) })).Invoke((object)default(object), new object[1] { level });
			}
			catch
			{
				try

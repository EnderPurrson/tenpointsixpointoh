...
namespace ExitGames.Client.Photon
{
	internal class NCommand : IComparable<NCommand>
	{
		...
		internal NCommand(EnetPeer peer, byte[] inBuff, ref int readingOffset)
		{
			...
			if (Payload != null)
			{
				Buffer.BlockCopy((global::System.Array)inBuff, readingOffset, (global::System.Array)Payload, 0, Payload.Length);
		...
		public override string ToString()
		{
			if (commandType == 1)
			{
				return string.Format("CMD({1} ack for c#:{0} s#/time {2}/{3})", new object[4] { commandChannelID, commandType, ackReceivedReliableSequenceNumber, ackReceivedSentTime });
			}
			return string.Format("CMD({1} c#:{0} r/u: {2}/{3} st/r#/rt:{4}/{5}/{6})", new object[7] { commandChannelID, commandType, reliableSequenceNumber, unreliableSequenceNumber, commandSentTime, commandSentCount, timeoutTime });
		}
	}
}

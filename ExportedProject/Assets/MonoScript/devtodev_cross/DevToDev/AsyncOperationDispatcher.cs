...
namespace DevToDev
{
	internal class AsyncOperationDispatcher : MonoBehaviour
	{
		...
		internal static AsyncOperationDispatcher Create()
		{
			...
			try
			{
				instance = Object.FindObjectOfType(typeof(AsyncOperationDispatcher)) as AsyncOperationDispatcher;
			}
			catch
			{
			}
			...
			gameObject.hideFlags = (HideFlags)11;
		...
		public void Dispose()
		{
			...
			{
				Object.Destroy(handeledGameObject);
			}
		}
		...
		public void Update()
		{
			...
			{
				Log.E("Message : " + ex.get_Message() + " trace: " + ex.get_StackTrace());
			}
			Action val = null;
			while ((val = executeOnMainThread.Dequeue()) != null)
			{
				try
				{
					val.Invoke();
				...
				{
					Log.E("Message : " + ex2.get_Message() + " trace: " + ex2.get_StackTrace());
				}
			}
		}
		...
		public void EndSession(string message)
		{
			...
			{
				long timestamp = 0L;
				if (message != null && long.TryParse(message, ref timestamp))
				{
					SDKClient.Instance.Suspend(timestamp);
				}
				else
				{
					SDKClient.Instance.Suspend(0L);

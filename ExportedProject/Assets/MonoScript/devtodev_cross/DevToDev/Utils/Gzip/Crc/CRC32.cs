...
namespace DevToDev.Utils.Gzip.Crc
{
	internal class CRC32
	{
		...
		public int GetCrc32AndCopy(Stream input, Stream output)
		{
			if (input == null)
			{
				throw new global::System.Exception("The input stream must not be null.");
			}
			byte[] array = new byte[8192];
			int num = 8192;
			_TotalBytesRead = 0L;
			int num2 = input.Read(array, 0, num);
			if (output != null)
			{
				output.Write(array, 0, num2);
			}
			_TotalBytesRead += num2;
			while (num2 > 0)
			{
				SlurpBlock(array, 0, num2);
				num2 = input.Read(array, 0, num);
				if (output != null)
				{
					output.Write(array, 0, num2);
				}
		...
		public void SlurpBlock(byte[] block, int offset, int count)
		{
			if (block == null)
			{
				throw new global::System.Exception("The data buffer must not be null.");
			}

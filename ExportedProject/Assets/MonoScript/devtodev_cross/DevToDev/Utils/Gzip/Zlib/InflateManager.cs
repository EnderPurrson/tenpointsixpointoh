			...
			while (true)
			{
				...
				case InflateManagerMode.METHOD:
					...
					if (((method = _codec.InputBuffer[_codec.NextIn++]) & 0xF) != 8)
					{
						...
						_codec.Message = string.Format("unknown compression method (0x{0:X2})", (object)method);
						marker = 5;
					}
					else if ((method >> 4) + 8 > wbits)
					{
						mode = InflateManagerMode.BAD;
						_codec.Message = string.Format("invalid window size ({0})", (object)((method >> 4) + 8));
						marker = 5;
					}
					else
					{
				...
				case InflateManagerMode.BAD:
					throw new ZlibException(string.Format("Bad state ({0})", (object)_codec.Message));
				default:
					{

...
internal sealed class ClanIncomingInvitesController : MonoBehaviour
{
	...
	private static System.Threading.Tasks.Task<List<object>> _currentRequest;

	private static CancellationTokenSource _cts;

	internal static System.Threading.Tasks.Task<List<object>> CurrentRequest
	{
		get
		{
	...
	internal static void FetchClanIncomingInvites(string playerId)
	{
		...
		_currentRequest = RequestClanIncomingInvitesAsync(playerId, _cts.get_Token());
	}
	...
	{
		//IL_0006: Unknown result type (might be due to invalid IL or missing references)
		return RequestClanIncomingInvitesAsync(playerId, 0f, CancellationToken.get_None());
	}
	...
	private static IEnumerator RequestClanIncomingInvitesCoroutine(string playerId, float delay, TaskCompletionSource<List<object>> promise, CancellationToken ct)
	{
		...
		if (((CancellationToken)(ref ct)).get_IsCancellationRequested())
		{
		...
		if (string.IsNullOrEmpty(FriendsController.sharedController.id))
		{
			...
			promise.TrySetException((Exception)new InvalidOperationException("Current player id is empty."));
			yield break;
		}
		...
		if (request == null)
		{
			promise.TrySetException((Exception)new InvalidOperationException("Request was not performed because player is connected to Photon."));
		...
		if (string.IsNullOrEmpty(responseText))
		{
			...
			promise.TrySetException((Exception)new InvalidOperationException("Clan incoming invites response is empty."));
			_currentRequest = RequestClanIncomingInvitesAsync(playerId, 10f, ct);
			yield break;
		}
		...
		if (d != null && d.TryGetValue("clans_invites", out invites))
		{
			...
			if (invites == null)
			{
				promise.TrySetException((Exception)new InvalidOperationException("“clans_invites” could not be parsed."));
			}
			else
			{
		...
		{
			promise.TrySetException((Exception)new InvalidOperationException("“clans_invites” not found."));
		}
	}
	...
	internal static void SetRequestDirty()
	{
		...
		if (!((System.Threading.Tasks.Task)_currentRequest).get_IsCompleted())
		{
	...
	private IEnumerator Start()
	{
		...
		else if (((System.Threading.Tasks.Task)CurrentRequest).get_IsCompleted() && (((System.Threading.Tasks.Task)CurrentRequest).get_IsCanceled() || ((System.Threading.Tasks.Task)CurrentRequest).get_IsFaulted()))
		{
			_cts = new CancellationTokenSource();
			_currentRequest = RequestClanIncomingInvitesAsync(playerId, _cts.get_Token());
		}
		...
		else if (((System.Threading.Tasks.Task)CurrentRequest).get_IsFaulted())
		{
		...
		{
			List<object> inviteList = CurrentRequest.get_Result();
			if (inviteList != null)
			{

...
namespace Facebook.Unity.Mobile.IOS
{
	internal class IOSFacebook : MobileFacebook
	{
		...
		public override void AppInvite(Uri appLinkUrl, Uri previewImageUrl, FacebookDelegate<IAppInviteResult> callback)
		{
			...
			if (appLinkUrl != (Uri)null && !string.IsNullOrEmpty(appLinkUrl.get_AbsoluteUri()))
			{
				appLinkUrl2 = appLinkUrl.get_AbsoluteUri();
			}
			if (previewImageUrl != (Uri)null && !string.IsNullOrEmpty(previewImageUrl.get_AbsoluteUri()))
			{
				previewImageUrl2 = previewImageUrl.get_AbsoluteUri();
			}
		...
		{
			string link2 = ((!(link != (Uri)null)) ? string.Empty : link.ToString());
			string picture2 = ((!(picture != (Uri)null)) ? string.Empty : picture.ToString());

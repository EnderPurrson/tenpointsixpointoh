...
internal sealed class FreeAwardController : MonoBehaviour
{
	...
	public sealed class WatchingState : State
	{
		...
		public WatchingState()
		{
			...
			else
			{
				...
				object result = FyberVideoLoaded.Result;
				Ad val = (Ad)((result is Ad) ? result : null);
				if (val == null)
				{
	...
	internal static Future<object> LoadFyberVideo(string callerName = null)
	{
		...
		onAdAvailable = delegate(Ad ad)
		{
			...
			if (Defs.IsDeveloperBuild)
			{
				Debug.LogFormat("LoadFyberVideo > AdAvailable: {{ format: {0}, placementId: '{1}' }}", ad.get_AdFormat(), ad.get_PlacementId());
			}
		...
		onRequestFail = delegate(RequestError requestError)
		{
			if (Defs.IsDeveloperBuild)
			{
				Debug.LogFormat("LoadFyberVideo > RequestFail: {{ requestError: {0} }}", requestError.get_Description());
			}
		...
		if (waitingState != null)
		{
			...
			else if (Provider == AdProvider.Fyber)
			{
				if (FyberVideoLoaded != null && FyberVideoLoaded.IsCompleted)
				{
					...
					object result2 = FyberVideoLoaded.Result;
					RequestError val2 = (RequestError)((result2 is RequestError) ? result2 : null);
					if (Defs.IsDeveloperBuild)
					{
						Debug.LogFormat("Fyber loading failed: {0}. Keep waiting.", (val2 != null) ? val2.get_Description() : ((!(FyberVideoLoaded.Result is AdFormat)) ? "?" : "Not available"));
					}

...
internal sealed class LeaderboardScript : MonoBehaviour
{
	...
	private void UpdateLocs()
	{
		...
		{
			TableFooterIndividual.transform.FindChild("LabelPlace").Map((Transform t) => t.gameObject.GetComponent<UILabel>()).Do(delegate(UILabel n)
			{
		...
		{
			TableFooterClan.transform.FindChild("LabelPlace").Map((Transform t) => t.gameObject.GetComponent<UILabel>()).Do(delegate(UILabel n)
			{
				n.text = LocalizationStore.Get("Key_0053");
			});
		}
	...
	private IEnumerator FillGrids(string response, string playerId, GridState state)
	{
		...
		try
		{
			...
			if (TableFooterIndividual != null)
			{
				...
				TableFooterIndividual.transform.FindChild("LabelNick").Map((Transform t) => t.gameObject.GetComponent<UILabel>()).Do(delegate(UILabel n)
				{
					n.text = me.Nickname;
				});
				TableFooterIndividual.transform.FindChild("LabelLevel").Map((Transform t) => t.gameObject.GetComponent<UILabel>()).Do(delegate(UILabel n)
				{
					n.text = me.Rank.ToString(CultureInfo.InvariantCulture);
				});
				TableFooterIndividual.transform.FindChild("LabelWins").Map((Transform t) => t.gameObject.GetComponent<UILabel>()).Do(delegate(UILabel w)
				{
					w.text = me.WinCount.ToString(CultureInfo.InvariantCulture);
				});
				UILabel clanLabel2 = TableFooterIndividual.transform.FindChild("LabelClan").Map((Transform t) => t.gameObject.GetComponent<UILabel>());
			...
			if (TableFooterClan != null)
			{
				...
				else
				{
					...
					if (myClanInTop == null)
					{
						...
						TableFooterClan.transform.FindChild("LabelMembers").Map((Transform t) => t.gameObject.GetComponent<UILabel>()).Do(delegate(UILabel n)
						{
							n.text = string.Empty;
						});
						TableFooterClan.transform.FindChild("LabelWins").Map((Transform t) => t.gameObject.GetComponent<UILabel>()).Do(delegate(UILabel w)
						{
							w.text = string.Empty;
						});
						UILabel clanLabel = TableFooterClan.transform.FindChild("LabelName").Map((Transform t) => t.gameObject.GetComponent<UILabel>());
	...
	internal void RefreshMyLeaderboardEntries()
	{
		...
		UILabel uILabel = TableFooterIndividual.transform.FindChild("LabelNick").Map((Transform t) => t.gameObject.GetComponent<UILabel>());
		if (uILabel != null)
		{
			uILabel.text = ProfileController.GetPlayerNameOrDefault();
		}
		UILabel uILabel2 = TableFooterIndividual.transform.FindChild("LabelClan").Map((Transform t) => t.gameObject.GetComponent<UILabel>());
		if (uILabel2 != null)
		{

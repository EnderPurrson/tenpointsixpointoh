...
internal sealed class MainMenuController : ControlsSettingsBase
{
	...
	private IEnumerator SynchronizeGoogleCoroutine(Action tryUpdateNickname, GameServicesController gameServicesController)
	{
		...
		try
		{
			...
			Action<bool> authenticateCallback = delegate(bool succeeded)
			{
				...
				{
					_syncPromise.TrySetResult(false);
					Debug.LogFormat("IsCompleted: {0} IsCanceled: {1} IsFaulted: {2}", SyncFuture.get_IsCompleted(), SyncFuture.get_IsCanceled(), SyncFuture.get_IsFaulted());
				}
				else
				{
					tryUpdateNickname2();
	...
	private new IEnumerator Start()
	{
		...
		if (trafficForwardingScript != null)
		{
			...
			System.Threading.Tasks.Task<TrafficForwardingInfo> trafficForwardingResult = trafficForwardingScript.GetTrafficForwardingInfo().Filter((System.Threading.Tasks.Task<TrafficForwardingInfo> t) => ((System.Threading.Tasks.Task)t).get_IsCompleted() && !((System.Threading.Tasks.Task)t).get_IsCanceled() && !((System.Threading.Tasks.Task)t).get_IsFaulted());
			if (trafficForwardingResult != null)
			{
				_trafficForwardingUrl = trafficForwardingResult.get_Result().Url;
			}
			if (trafficForwardingButton != null)
			{
				RefreshTrafficForwardingButton(this, (trafficForwardingResult == null) ? TrafficForwardingInfo.DisabledInstance : trafficForwardingResult.get_Result());
			}
		}
		...
		if (skinsMakerButton != null)
		{
			...
			{
				ButtonHandler bh11 = skinsMakerButton.GetComponent<ButtonHandler>();
				if (bh11 != null)
				{
					bh11.Clicked += HandleSkinsMakerClicked;
				}
			}
			else
			{
		...
		{
			ButtonHandler bh10 = profileButton.GetComponent<ButtonHandler>();
			if (bh10 != null)
			{
				bh10.Clicked += HandleProfileClicked;
			}
		}
		if (freeButton != null)
		{
			ButtonHandler bh9 = freeButton.GetComponent<ButtonHandler>();
			if (bh9 != null)
			{
				bh9.Clicked += HandleFreeClicked;
			}
		}
		if (gameCenterButton != null)
		{
			...
			if (gameServicesButton != null)
			{
				...
				ButtonHandler bh8 = gameCenterButton.GetComponent<ButtonHandler>();
				if (bh8 != null)
				{
					bh8.Clicked += HandleGameServicesClicked;
				}
			}
		}
		if (signOutButton != null)
	...
	private void Update()
	{
		...
		if (_newClanIncomingInvitesSprite.Value != null)
		{
			...
			{
				_newClanIncomingInvitesSprite.Value.gameObject.SetActive(ClanIncomingInvitesController.CurrentRequest.get_Result().Count > 0);
			}
		}
	}
	...
	private void ReturnPersTonNormState()
	{
		...
		HOTween.Kill((object)pers);
		Vector3 vector = new Vector3(-0.33f, 28f, -0.28f);
		idleTimerLastTime = Time.realtimeSinceStartup;
		HOTween.To((object)pers, 0.5f, new TweenParms().Prop("localRotation", (ABSTweenPlugin)new PlugQuaternion(vector)).Ease((EaseType)0).OnComplete((TweenCallback)delegate
		{
			idleTimerLastTime = Time.realtimeSinceStartup;
		}));
	}
	...
	private void OnStarterPackContainerShow(bool enable)
	{
		...
		bool flag = (task == null || !TrafficForwardingEnabled(task.get_Result())) && enable;
	...
	public void HandleTrafficForwardingClicked()
	{
		...
		finally
		{
			...
			if (trafficForwardingScript != null)
			{
				...
				TrafficForwardingInfo e = ((!((System.Threading.Tasks.Task)trafficForwardingInfo).get_IsCompleted() || ((System.Threading.Tasks.Task)trafficForwardingInfo).get_IsCanceled() || ((System.Threading.Tasks.Task)trafficForwardingInfo).get_IsFaulted()) ? TrafficForwardingInfo.DisabledInstance : trafficForwardingInfo.get_Result());

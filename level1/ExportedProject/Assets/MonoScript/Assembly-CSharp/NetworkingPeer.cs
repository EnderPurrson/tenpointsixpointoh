...
internal class NetworkingPeer : LoadBalancingPeer, IPhotonPeerListener
{
	...
	public bool ConnectToRegionMaster(CloudRegionCode region)
	{
		...
		if (State == ClientState.ConnectedToNameServer)
		{
			...
			return OpAuthenticate(AppId, AppVersion, authValues, region.ToString(), requestLobbyStatistics);
		}
		string nameServerAddress = NameServerAddress;
		if (!base.Connect(nameServerAddress, "ns"))
		{
	...
	public void OnStatusChanged(StatusCode statusCode)
	{
		...
		case StatusCode.Connect:
			...
			{
				AuthenticationValues authenticationValues4 = AuthValues;
				...
				didAuthenticate = OpAuthenticate(AppId, AppVersion, authValues3, CloudRegion.ToString(), requestLobbyStatistics);
				if (didAuthenticate)
				{
					State = ClientState.Authenticating;
				}
		...
		case StatusCode.EncryptionEstablished:
			...
			if (!didAuthenticate && (!IsUsingNameServer || CloudRegion != CloudRegionCode.none))
			{
				...
				AuthenticationValues authValues = authenticationValues;
				didAuthenticate = OpAuthenticate(AppId, AppVersion, authValues, CloudRegion.ToString(), requestLobbyStatistics);
				if (didAuthenticate)
				{
					State = ClientState.Authenticating;
				}
		...
		case StatusCode.EncryptionFailedToEstablish:
		{
			...
			AuthenticationValues authenticationValues3 = AuthValues;
			...
			OpAuthenticate(AppId, AppVersion, authValues2, CloudRegion.ToString(), requestLobbyStatistics);
			break;
		}
		...
		case StatusCode.TimeoutDisconnect:
			...
			if (base.ServerAddress.Equals(GameServerAddress))
			{
	...
	private object[] DeltaCompressionWrite(object[] previousContent, object[] currentContent)
	{
		...
		for (int i = 3; i < currentContent.Length; i++)
		{
			...
			if (obj == null)
			{
				if (val == null)
				{
					val = new Queue<int>(currentContent.Length);
				}
				val.Enqueue(i);
			}
		}

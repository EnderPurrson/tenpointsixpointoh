...
public class PhotonTransformViewPositionControl
{
	...
	private Vector3 GetOldestStoredNetworkPosition()
	{
		...
		if (m_OldNetworkPositions.get_Count() > 0)
		{
	...
	public Vector3 UpdatePosition(Vector3 currentPosition)
	{
		...
		case PhotonTransformViewPositionModel.InterpolateOptions.EstimatedSpeed:
			if (m_OldNetworkPositions.get_Count() != 0)
			{
				float num = Vector3.Distance(m_NetworkPosition, GetOldestStoredNetworkPosition()) / (float)m_OldNetworkPositions.get_Count() * (float)PhotonNetwork.sendRateOnSerialize;
	...
	private void DeserializeData(PhotonStream stream, PhotonMessageInfo info)
	{
		...
		m_NetworkPosition = networkPosition;
		while (m_OldNetworkPositions.get_Count() > m_Model.ExtrapolateNumberOfStoredPositions)
		{

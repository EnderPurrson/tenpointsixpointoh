...
public sealed class Player_move_c : MonoBehaviour
{
	...
	private void UpdateHealth()
	{
		...
		if (isMulti)
		{
			...
			TweenParms val = new TweenParms().Prop("localPosition", (object)new Vector3(localPosition.x, 100f, localPosition.z)).Ease((EaseType)7).OnComplete((TweenCallback)delegate
			{
			...
			HOTween.To((object)myPlayerTransform, (!isNeedShowRespawnWindow) ? 2f : 0.75f, val);
			return;
		}
		if (Defs.IsSurvival)
		{
	...
	public void StartLoopShot()
	{
		...
		StartCoroutine(ShootLoop(ctsShootLoop.get_Token(), lengthAnimShootDown));
	}
	...
	private IEnumerator ShootLoop(CancellationToken token, float _lengthAnimShootDown)
	{
		...
		if (!((CancellationToken)(ref token)).get_IsCancellationRequested())
		{
		...
		if (!Defs.isMulti || isMine)
		{
			while (!((CancellationToken)(ref token)).get_IsCancellationRequested())
			{
				shootS();

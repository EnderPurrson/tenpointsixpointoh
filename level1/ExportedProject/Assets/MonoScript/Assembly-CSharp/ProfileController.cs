...
internal sealed class ProfileController : MonoBehaviour
{
	...
	{
		if (!(profileView != null) || !InterfaceEnabled || HOTween.IsTweening((object)profileView.characterView.character))
		{
			return;
		}
		float num = -120f;
	...
	private void ReturnCharacterToInitialState()
	{
		...
		int num = HOTween.Kill((object)profileView.characterView.character);
		if (num > 0 && Application.isEditor)
		{
			Debug.LogWarning("Tweens killed: " + num);
		}
		_idleTimeStart = Time.realtimeSinceStartup;
		HOTween.To((object)profileView.characterView.character, 0.5f, new TweenParms().Prop("localRotation", (ABSTweenPlugin)new PlugQuaternion(_initialLocalRotation)).UpdateType((UpdateType)3).Ease((EaseType)0)

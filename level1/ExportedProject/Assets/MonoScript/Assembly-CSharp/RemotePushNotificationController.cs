...
public sealed class RemotePushNotificationController : MonoBehaviour
{
	...
	private IEnumerator Start()
	{
		...
		try
		{
			...
			ScopeLogger scopeLogger2 = new ScopeLogger(thisMethod, "GoogleCloudMessaging.register()", Defs.IsDeveloperBuild);
			try
			{
	...
	private IEnumerator ReciveUpdateDataToServer(string deviceToken)
	{
		...
		try
		{
			...
			WWW request = Tools.CreateWwwIfNotConnected(UrlPushNotificationServer, form, "RemotePushNotificationController.ReciveUpdateDataToServer()", headers);
			if (request == null)
			{
				yield break;
			}

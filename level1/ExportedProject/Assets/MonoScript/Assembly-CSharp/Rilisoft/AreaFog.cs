...
namespace Rilisoft
{
	public class AreaFog : AreaBase
	{
		...
		public override void CheckIn(GameObject to)
		{
			...
			StartCoroutine(Change(_settings, animationTime, _tokenSource.get_Token()));
		}
		...
		public override void CheckOut(GameObject from)
		{
			...
			_tokenSource.Cancel();
			_tokenSource = new CancellationTokenSource();
			StartCoroutine(Change(_prevSettings, animationTime, _tokenSource.get_Token()));
		}
		...
		private IEnumerator Change(FogSettings to, float time, CancellationToken token)
		{
			...
			if (RenderSettings.fog)
			{
				...
				float elapsed = 0f;
				while (elapsed < time && !((CancellationToken)(ref token)).get_IsCancellationRequested())
				{

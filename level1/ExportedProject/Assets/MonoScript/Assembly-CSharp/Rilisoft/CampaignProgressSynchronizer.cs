	...
	internal sealed class CampaignProgressSynchronizer
	{
		...
		private IEnumerator SyncGoogleCoroutine(bool pullOnly)
		{
			...
			try
			{
				...
				for (int i = 0; i != 3; i++)
				{
					System.Threading.Tasks.Task<GoogleSavedGameRequestResult<CampaignProgressMemento>> future = googleSavedGamesFacade.Pull();
					...
					CampaignProgressMemento cloudCampaignProgress = future.get_Result().Value;
		...
		private IEnumerator PushGoogleCoroutine()
		{
			...
			try
			{
				...
				for (int i = 0; i != 3; i++)
				{
					...
					System.Threading.Tasks.Task<GoogleSavedGameRequestResult<ISavedGameMetadata>> future = googleSavedGamesFacade.Push(localCampaignProgress);
					while (!((System.Threading.Tasks.Task)future).get_IsCompleted())
					{
						yield return null;
					}

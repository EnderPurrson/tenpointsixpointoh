...
namespace Rilisoft
{
	internal struct CampaignProgressSynchronizerGpgFacade
	{
		...
		public System.Threading.Tasks.Task<GoogleSavedGameRequestResult<ISavedGameMetadata>> Push(CampaignProgressMemento campaignProgress)
		{
			...
			try
			{
				...
				try
				{
					PushCallback pushCallback = new PushCallback(campaignProgress, val);
		...
		public System.Threading.Tasks.Task<GoogleSavedGameRequestResult<CampaignProgressMemento>> Pull()
		{
			...
			try
			{
				TaskCompletionSource<GoogleSavedGameRequestResult<CampaignProgressMemento>> val = new TaskCompletionSource<GoogleSavedGameRequestResult<CampaignProgressMemento>>();
				ScopeLogger scopeLogger2 = new ScopeLogger(text, "OpenWithManualConflictResolution", Defs.IsDeveloperBuild);
				try
				{

...
namespace Rilisoft
{
	internal sealed class PersistentCacheManager : MonoBehaviour
	{
		...
		internal System.Threading.Tasks.Task FirstResponse
		{
			get
			{
				return _firstResponsePromise.get_Task();
			}
		}
		...
		public System.Threading.Tasks.Task StartDownloadSignaturesLoop()
		{
			...
			if (!IsEnabled)
			{
				...
				return firstResponsePromise.get_Task();
			}
			_cancellationTokenSource.Cancel();
			_cancellationTokenSource = new CancellationTokenSource();
			float delaySeconds = (Application.isEditor ? 30f : ((!Defs.IsDeveloperBuild) ? 600f : 60f));
			StartCoroutine(DownloadSignaturesLoopCoroutine(delaySeconds, _cancellationTokenSource.get_Token(), firstResponsePromise));
			return firstResponsePromise.get_Task();
		}
		...
		private IEnumerator DownloadSignaturesLoopCoroutine(float delaySeconds, CancellationToken cancellationToken, TaskCompletionSource<Dictionary<string, string>> promise)
		{
			...
			int i = 0;
			while (!((CancellationToken)(ref cancellationToken)).get_IsCancellationRequested())
			{
		...
		private IEnumerator DownloadSignaturesOnceCoroutine(CancellationToken cancellationToken, TaskCompletionSource<Dictionary<string, string>> promise, int context = 0)
		{
			...
			yield return StartCoroutine(WaitCompletionAndParseSignaturesCoroutine(signaturesStringPromise.get_Task(), promise));
			if (((System.Threading.Tasks.Task)promise.get_Task()).get_IsCanceled())
			{
				Debug.LogWarningFormat("DownloadSignaturesOnceCoroutine({0}) cancelled.", context);
				yield break;
			}
			if (((System.Threading.Tasks.Task)promise.get_Task()).get_IsFaulted())
			{
				Debug.LogWarningFormat("DownloadSignaturesOnceCoroutine({0}) failed: {1}", context, ((Exception)(object)((System.Threading.Tasks.Task)promise.get_Task()).get_Exception()).InnerException);
				string[] array = urls;
			...
			Dictionary<string, string> d = promise.get_Task().get_Result();
			if (Defs.IsDeveloperBuild)
			{
		...
		private static IEnumerator WaitCompletionAndParseSignaturesCoroutine(System.Threading.Tasks.Task<string> future, TaskCompletionSource<Dictionary<string, string>> promise)
		{
			try
			{
				...
				yield return new WaitUntil(() => ((System.Threading.Tasks.Task)future2).get_IsCompleted());
				if (((System.Threading.Tasks.Task)future).get_IsCanceled())
				{
				...
				{
					promise.TrySetException(((System.Threading.Tasks.Task)future).get_Exception());
					yield break;
				}
				Dictionary<string, string> result = new Dictionary<string, string>();
				Dictionary<string, object> signaturesDictionaryObjects = Json.Deserialize(future.get_Result()) as Dictionary<string, object>;
		...
		private static IEnumerator DownloadSignaturesCoroutine(string[] urls, CancellationToken cancellationToken, TaskCompletionSource<string> promise)
		{
			...
			try
			{
				...
				{
					yield return null;
					if (((CancellationToken)(ref cancellationToken)).get_IsCancellationRequested())
					{
						promise.TrySetCanceled();
						yield break;
					}

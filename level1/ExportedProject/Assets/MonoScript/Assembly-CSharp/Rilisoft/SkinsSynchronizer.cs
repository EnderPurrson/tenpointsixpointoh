		...
		private IEnumerator SyncGoogleCoroutine(bool pullOnly)
		{
			...
			try
			{
				...
				while (true)
				{
					...
					{
						System.Threading.Tasks.Task<GoogleSavedGameRequestResult<SkinsMemento>> future = googleSavedGamesFacade.Pull();
						...
						else
						{
							...
							{
								SkinsMemento cloudSkins = future.get_Result().Value;
		...
		internal IEnumerator PushGoogleCoroutine()
		{
			...
			try
			{
				...
				while (true)
				{
					...
					using (ScopeLogger logger = new ScopeLogger(thisMethod, callee, Defs.IsDeveloperBuild && !Application.isEditor))
					{
						System.Threading.Tasks.Task<GoogleSavedGameRequestResult<ISavedGameMetadata>> future = googleSavedGamesFacade.Push(localSkins);
						while (!((System.Threading.Tasks.Task)future).get_IsCompleted())
						{
							yield return null;
						}

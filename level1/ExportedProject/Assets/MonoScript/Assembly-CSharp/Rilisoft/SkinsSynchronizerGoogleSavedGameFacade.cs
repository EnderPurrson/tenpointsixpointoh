...
namespace Rilisoft
{
	public struct SkinsSynchronizerGoogleSavedGameFacade
	{
		...
		public System.Threading.Tasks.Task<GoogleSavedGameRequestResult<ISavedGameMetadata>> Push(SkinsMemento skins)
		{
			...
			try
			{
				...
				try
				{
					PushCallback pushCallback = new PushCallback(skins, val);
		...
		public System.Threading.Tasks.Task<GoogleSavedGameRequestResult<SkinsMemento>> Pull()
		{
			...
			try
			{
				TaskCompletionSource<GoogleSavedGameRequestResult<SkinsMemento>> val = new TaskCompletionSource<GoogleSavedGameRequestResult<SkinsMemento>>();
				if (SavedGame == null)
				{
					val.TrySetException((Exception)new InvalidOperationException("SavedGameClient is null."));
					return val.get_Task();

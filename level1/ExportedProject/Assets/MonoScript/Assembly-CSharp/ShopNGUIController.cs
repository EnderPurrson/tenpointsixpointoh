...
public class ShopNGUIController : MonoBehaviour
{
	...
	private void FilterUpgrades(List<GameObject> modelsList, GameObject prefab, CategoryNames category, string visualDefName)
	{
		...
		List<string> list2 = list.FirstOrDefault((List<string> l) => l.Contains(prefab.name)).ToList();
		if (list2 == null)
		{
			return;
		}
		string text = ((!string.IsNullOrEmpty(visualDefName)) ? Storager.getString(visualDefName, false) : string.Empty);
		int num = list2.IndexOf(prefab.name);
		...
		else if (!string.IsNullOrEmpty(text) && prefab.name.Equals(text))
		{
			modelsList.Add(prefab);
		}
	}
	...
	public List<GameObject> FillModelsList(CategoryNames c)
	{
		...
		else
		{
			...
			case CategoryNames.ArmorCategory:
				...
				{
					foreach (ShopPositionParams item2 in armor)
					{
						FilterUpgrades(list, item2.gameObject, CategoryNames.ArmorCategory, Defs.VisualArmor);
					}
				}
	...
	public void SetCamera()
	{
		...
		HOTween.Kill((object)mainMenu_Pers);
		...
		HOTween.To((object)mainMenu_Pers, num, new TweenParms().Prop("localPosition", (object)vector).Prop("localRotation", (ABSTweenPlugin)new PlugQuaternion(vector2)).UpdateType((UpdateType)3)
			.Ease((EaseType)0)
	...
	public void UpdateIcon(CategoryNames c, bool animateModel = false)
	{
		...
		{
			List<GameObject> list = FillModelsList(c);
			GameObject gameObject = ((c != CategoryNames.SkinsCategory) ? ItemDb.GetWearFromResources("cape_Custom", CategoryNames.CapesCategory) : list[0]);
			if (gameObject != null)
			{
				AddModel(gameObject, delegate(GameObject manipulateObject, Vector3 positionShop, Vector3 rotationShop, string readableName, float sc, int _unusedTier, int _unusedLeague)
				{
					...
					if (animateModel)
					{
						...
						HOTween.To((object)transform, 0.25f, new TweenParms().Prop("localScale", (object)localScale).UpdateType((UpdateType)3).Ease((EaseType)0)
							.OnComplete((TweenCallback)delegate
							{
							}));
					}
		...
		if (texture != null)
		{
			Player_move_c.PerformActionRecurs(tb.gameObject, delegate(Transform ch)
			{
				...
				else if (ch.gameObject.name.Equals("ShopIcon"))
				{
					...
					if (component.mainTexture == null || !component.mainTexture.name.Equals(texture.name) || HOTween.IsTweening((object)ch))
					{
						HOTween.Kill((object)ch);
						ch.localScale = new Vector3(1.25f, 1.25f, 1.25f);
						HOTween.To((object)ch, 0.25f, new TweenParms().Prop("localScale", (object)new Vector3(1f, 1f, 1f)).UpdateType((UpdateType)3).Ease((EaseType)0)
							.OnComplete((TweenCallback)delegate
							{
							}));
					}
	...
	internal void ReloadCarousel(string idToChoose = null)
	{
		...
		if (currentCategory == CategoryNames.SkinsCategory)
		{
			...
			long result;
			int num = list2.FindIndex((string kvp) => long.TryParse(kvp, out result) && result >= 1000000);
			int num2 = 0;
		...
		if (idToChoose != null)
		{
			if (WeaponCategory)
			{
				ItemRecord itemRecord2 = ItemDb.GetByTag(idToChoose);
				num3 = ((itemRecord2 == null) ? (-1) : list.FindIndex((GameObject go) => go.nameNoClone() == itemRecord2.PrefabName));
			}
			else if (currentCategory != CategoryNames.SkinsCategory)
			{
				num3 = list.FindIndex((GameObject go) => go.nameNoClone() == idToChoose);
			}
		}
		for (int m = 0; m < list.Count; m++)
		{
			...
			GameObject pref = list[m];
			gameObject2.name = m.ToString("D7");
			if (WeaponCategory)
			{
				gameObject2.name = gameObject2.name + "_" + int.Parse(pref.name.Substring("Weapon".Length)).ToString("D5");
			}
			ShopCarouselElement sce = gameObject2.GetComponent<ShopCarouselElement>();
			string itenID = (WeaponCategory ? ItemDb.GetByPrefabName(pref.name.Replace("(Clone)", string.Empty)).Tag : ((currentCategory != CategoryNames.SkinsCategory) ? ItemIDForPrefabReverse(pref.name, currentCategory) : array2[m]));
			sce.itemID = itenID;
			...
			{
				isBought = Storager.getInt(WeaponManager.storeIDtoDefsSNMapping[WeaponManager.tagToStoreIDMapping[sce.itemID]], true) > 0;
			}
			if (currentCategory == CategoryNames.SkinsCategory)
			{
			...
			Action<GameObject, CategoryNames> action = delegate(GameObject loadedOBject, CategoryNames category)
			{
				AddModel(loadedOBject, delegate(GameObject manipulateObject, Vector3 positionShop, Vector3 rotationShop, string readableName, float scaleCoefShop, int tier, int league)
				{
					...
					else
					{
						...
						if (ExpController.Instance != null && ExpController.Instance.OurTier < tier && tier < 100 && ((IsWeaponCategory(category) && sce.itemID.Equals(WeaponManager.FirstUnboughtOrForOurTier(sce.itemID)) && !isBought) || (IsWearCategory(category) && sce.itemID.Equals(WeaponManager.FirstUnboughtTag(sce.itemID)) && sce.itemID != "cape_Custom" && sce.itemID != "boots_tabi")) && sce.locked != null)
						{
							sce.locked.SetActive(true);
						}
						if ((IsWeaponCategory(category) && !sce.itemID.Equals(WeaponManager.FirstUnboughtOrForOurTier(sce.itemID)) && tier < 100) || (IsWearCategory(category) && !sce.itemID.Equals(WeaponManager.FirstUnboughtTag(sce.itemID))))
						{
							if (sce.arrow != null)
							{
				...
				}, category, false, (!IsWeaponCategory(category)) ? null : pref.GetComponent<WeaponSounds>());
			};
			int num4 = -1;
			if (WeaponCategory)
			{
				ItemRecord itemRecord = ItemDb.GetByTag(itenID);
				num4 = ((itemRecord == null) ? (-1) : list.FindIndex((GameObject go) => go.name.Replace("(Clone)", string.Empty) == itemRecord.PrefabName));
			}
			else if (currentCategory != CategoryNames.SkinsCategory)
			{
				num4 = list.FindIndex((GameObject go) => go.nameNoClone() == itenID);
			}
			if ((num4 < 0 || num3 < 0 || Mathf.Abs(num3 - num4) > 2) && currentCategory != CategoryNames.SkinsCategory && !EnableConfigurePos)
			{
				CoroutineRunner.Instance.StartCoroutine(LoadModelAsync(action, pref, currentCategory));
			}
			else
			{
				action(WeaponCategory ? WeaponManager.InnerPrefabForWeaponSync(pref.nameNoClone()) : ((currentCategory == CategoryNames.SkinsCategory) ? pref : ItemDb.GetWearFromResources(pref.nameNoClone(), currentCategory)), currentCategory);
			}
		}
		wrapContent.Reposition();
		ChooseCarouselItem(idToChoose, true);
	}
	...
	public void CategoryChosen(CategoryNames i, string idToSet = null, bool initial = false)
	{
		...
		else
		{
			...
			case CategoryNames.HatsCategory:
			{
				...
				int num3 = hats.FindIndex((ShopPositionParams go) => go.name.Equals(fu));
	...
	private void Update()
	{
		...
		if (mainPanel.activeInHierarchy && !HOTween.IsTweening((object)MainMenu_Pers))
		{
			float num = -120f;

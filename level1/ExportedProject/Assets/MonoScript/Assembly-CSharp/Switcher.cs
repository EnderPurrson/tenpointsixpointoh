...
internal sealed class Switcher : MonoBehaviour
{
	...
	internal static IEnumerable<float> InitializeStorager()
	{
		...
		{
			Debug.LogError("Exception in Trying to unequip armor hat or temp armor hat (mistakenly got from gocha as a gift): " + e);
		}
		...
		progress3 = 0.25f;
		if (Application.isEditor || (Application.platform == RuntimePlatform.IPhonePlayer && Debug.isDebugBuild) || (Application.platform == RuntimePlatform.IPhonePlayer && !Storager.hasKey(Defs.initValsInKeychain17)))
		{
		...
		Debug.Log(" StartTimeShowBannersString=" + Defs.StartTimeShowBannersString);
		...
		if (!DateTimeOffset.TryParse(lastTimeUpdateShownString, out lastTimeUpdateShown) && Debug.isDebugBuild)
		{
			Debug.LogWarning("Cannot parse " + lastTimeUpdateShownString);
		}
	...
	{
		Debug.LogFormat("> Switcher.Start(): {0:f3}, {1}", Time.realtimeSinceStartup, Time.frameCount);
		yield return StartCoroutine(ParseConfigsCoroutine());
		if (!TrainingController.TrainingCompleted && TrainingController.CompletedTrainingStage == TrainingController.NewTrainingCompletedStage.None)
		{
			PlayComicsSound();
		}
		Debug.Log("Switcher.Start() > InitializeSwitcher()");
	...
	public IEnumerable<float> InitializeSwitcher(Action setArmorArmy1ComesFromCloud = null)
	{
		...
		if (Defs.AndroidEdition == Defs.RuntimeAndroidEdition.GoogleLite)
		{
			try
			{
				...
				{
					Debug.Log("Switcher: Trying to initialize Google Play Games...");
				}
		...
		if (!PlayerPrefs.HasKey(Defs.SwitchingWeaponsSwipeRegimSN))
		{
			...
			if (Screen.dpi > 0f)
			{
				...
				{
					Debug.Log(string.Format("Device dpi: {0},    diagonal: {1} px ({2}\")", Screen.dpi, diagonalInPixels, diagonalInInches));
				}
				switchingWeaponMode = ((!(diagonalInInches < 6.8)) ? 1 : 0);
			}
			else if (Debug.isDebugBuild)
			{
				Debug.Log(string.Format("Device dpi: {0},    diagonal: {1} px", Screen.dpi, diagonalInPixels));
			}
		...
		{
			Debug.LogError("Switcher.InitializeSwitcher():    nickStackPrefab == null");
		}
		...
		{
			Debug.LogError("Switcher.InitializeSwitcher():    sceneInfoController == null");
		}
		else
		{
		...
		{
			Debug.LogError("Switcher.InitializeSwitcher():    ExperienceControllerPrefab == null");
		}
		...
		{
			Debug.LogWarning("ExperienceGuiPrefab == null");
		}
		_progress = bounds.Clamp(_progress + 0.01f);
		yield return _progress;
		...
		{
			Debug.LogWarning("BankGuiPrefab == null");
		}
		_progress = bounds.Clamp(_progress + 0.01f);
		yield return _progress;
		...
		{
			Debug.LogWarning("freeAwardGuiPrefab == null");
		}
		_progress = bounds.Clamp(_progress + 0.01f);
		yield return _progress;
		...
		if (Defs.IsDeveloperBuild)
		{
			Debug.LogFormat("Persistent cache: '{0}'", persistentCache.PersistentDataPath);
		}
		_progress = bounds.Clamp(_progress + 0.01f);
		yield return _progress;
		...
		if (Application.platform == RuntimePlatform.Android)
		{
			try
			{
				...
				if (Defs.IsDeveloperBuild)
				{
					Debug.LogFormat("[Rilisoft] Loading classes.dex, frame: {0}, time: {1:0.000}s", Time.frameCount, startLoadingDex);
				}
				...
				if (Defs.IsDeveloperBuild)
				{
					Debug.LogFormat("[Rilisoft] Loaded classes.dex in {0:0.000}s", Time.realtimeSinceStartup - startLoadingDex);
				}
			}
			catch (Exception ex4)
			{
				Exception ex2 = ex4;
				Debug.LogWarningFormat("[Rilisoft] Failed loading classes.dex: {0}", ex2.Message);
			}
		}
		_progress = bounds.Clamp(_progress + 0.01f);
		yield return _progress;
		...
		foreach (float item6 in InitializeStorager())
		{
			...
			if (_stopwatch.get_ElapsedMilliseconds() > 100)
			{
				_stopwatch.Reset();
				_stopwatch.Start();
				yield return _progress;
		...
		if (BuildSettings.BuildTargetPlatform == RuntimePlatform.IPhonePlayer)
		{
			...
			{
				Debug.LogError("SetRememberedTiersForWeaponsComesFromCloud exception: " + e);
			}
			_progress = bounds.Clamp(_progress + 0.01f);
			yield return _progress;
		...
		if (Defs.IsDeveloperBuild)
		{
			Debug.LogFormat("Loading {0:P1} > Instantiate License Verification Controller: {1}", _progress, needInstantiateLicenseVerification);
		}
		...
		if (Defs.IsDeveloperBuild)
		{
			Debug.LogFormat("Loading {0:P1} > Instantiate Temp Items: {1}", _progress, needInstantiateTempItems);
		}
		...
		if (Defs.IsDeveloperBuild)
		{
			Debug.LogFormat("Loading {0:P1} > Instantiate TwitterController.", _progress);
		}
	...
	public static void PlayComicsSound()
	{
		if (!(comicsSound != null))
		{
			...
			if (gameObject == null)
			{
				Debug.LogWarning("ComicsSoundPrefab is null.");
				return;
			}
	...
	private static void CheckHugeUpgrade()
	{
		...
		if (flag && !flag2)
		{
			...
			Debug.LogError("Upgrade tampering detected: " + AbuseMethod);
		}
	}
	...
	private static void PerformEssentialInitialization(string currencyKey, AbuseMetod abuseMethod)
	{
		...
		if (DigestStorager.Instance.ContainsKey(currencyKey))
		{
			if (!DigestStorager.Instance.Verify(currencyKey, @int))
			{
				...
				Debug.LogError("Currency tampering detected: " + AbuseMethod);
			}
		}
		else
		{
	...
	private static void PerformWeaponInitialization()
	{
		...
		if (DigestStorager.Instance.ContainsKey("WeaponsCount"))
		{
			if (!DigestStorager.Instance.Verify("WeaponsCount", value))
			{
				...
				Debug.LogError("Weapon tampering detected: " + AbuseMethod);
			}
		}
		else
		{
	...
	private static void PerformExpendablesInitialization()
	{
		...
		if (DigestStorager.Instance.ContainsKey("ExpendablesCount"))
		{
			if (!DigestStorager.Instance.Verify("ExpendablesCount", value))
			{
				...
				Debug.LogError("Expendables tampering detected: " + AbuseMethod);
			}
		}
		else
		{

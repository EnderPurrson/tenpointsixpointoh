...
public sealed class WeaponManager : MonoBehaviour
{
	...
	private void SaveTryGunsDiscounts()
	{
		try
		{
			...
			Storager.setString("WeaponManager.TryGunsDiscountsValuesKey", Json.Serialize(tryGunDiscounts.ToDictionary((KeyValuePair<string, SaltedLong> kvp) => kvp.Key, (KeyValuePair<string, SaltedLong> kvp) => kvp.Value.Value)), false);
		}
		catch (Exception ex)
		{
	...
	private IEnumerator GetWeaponPrefabsCoroutine(int filterMap = 0)
	{
		...
		foreach (WeaponSounds ws2 in outerWeaponPrefabs)
		{
			...
			if (!Device.isPixelGunLow)
			{
				removedAndNotBoughtGun = removedAndNotBought(ws2);
			}
			bool loadInnerPrefab = (!Device.IsLoweMemoryDevice || tierCondition(ws2)) && !removedAndNotBoughtGun;
	...
	{
		return (from ws in AllWrapperPrefabs()
			select ws.gameObject).FirstOrDefault((GameObject w) => ItemDb.GetByPrefabName(w.name).Tag.Equals(weaponTag));
	}
	...
	public static bool RemoveGunFromAllTryGunRelated(string tg)
	{
		...
		string lastBought = LastBoughtTag(tg);
		if (lastBought == null)
		{
			Debug.LogError("RemoveGunFromAllTryGunRelated: lastBought == null,  tg = " + tg);
			return false;
		}
		bool result = sharedManager.TryGuns.Remove(lastBought);
		sharedManager.ExpiredTryGuns.RemoveAll((string expiredGunTag) => expiredGunTag == lastBought);
		sharedManager.RemoveDiscountForTryGun(lastBought);
		return result;
	}

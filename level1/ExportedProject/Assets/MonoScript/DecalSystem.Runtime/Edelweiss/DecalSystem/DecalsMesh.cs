...
namespace Edelweiss.DecalSystem
{
	public class DecalsMesh : GenericDecalsMesh<Decals, DecalProjectorBase, DecalsMesh>
	{
		...
		public void Add(Vector3[] a_Vertices, Vector3[] a_Normals, Vector4[] a_Tangents, Vector2[] a_UVs, Vector2[] a_UV2s, int[] a_Triangles, Matrix4x4 a_WorldToMeshMatrix, Matrix4x4 a_MeshToWorldMatrix)
		{
			...
			else
			{
				...
				Debug.Log(string.Concat(new object[7]
				{
					num,
					", ",
					a_WorldToMeshMatrix[0, 0],
					", ",
					a_WorldToMeshMatrix[1, 1],
					", ",
					a_WorldToMeshMatrix[2, 2]
				}));
		...
		public void Add(Terrain a_Terrain, Matrix4x4 a_TerrainToDecalsMatrix)
		{
			...
			int count = base.Vertices.get_Count();
			int count2 = base.Triangles.get_Count();
			if (num < num3 && num2 < num4)
			{
				...
				for (int i = num; i <= num3; i++)
				{
					...
					for (int j = num2; j <= num4; j++)
					{
						...
						Vector3 vector = a_TerrainToDecalsMatrix.MultiplyPoint3x4(new Vector3(x2, height, z2));
				...
				for (int k = 0; k < num9; k++)
				{
					for (int l = 0; l < num10; l++)
					{
						...
						m_Triangles.Add(num14);
						m_Triangles.Add(num11);
						m_Triangles.Add(num14);
						m_Triangles.Add(num13);
					}
				}
			}
			float num15 = Mathf.Cos(activeDecalProjector.CullingAngle * ((float)Math.PI / 180f));
			if (!Mathf.Approximately(num15, -1f))
			{
				Vector3 v = new Vector3(0f, 1f, 0f);
				Matrix4x4 worldToLocalMatrix = m_Decals.CachedTransform.worldToLocalMatrix;
				v = (activeDecalProjector.ProjectorToWorldMatrix * worldToLocalMatrix).inverse.transpose.MultiplyVector(v).normalized;
				for (int m = count; m < m_Vertices.get_Count(); m++)
				{
					Vector3 rhs = m_Normals.get_Item(m);
					if (num15 > Vector3.Dot(v, rhs))
					{
						m_RemovedIndices.AddRemovedIndex(m);
					}
				}
				for (int num16 = m_Triangles.get_Count() - 1; num16 >= count2; num16 -= 3)
				{
					int a_Index = m_Triangles.get_Item(num16);
					int a_Index2 = m_Triangles.get_Item(num16 - 1);
					int a_Index3 = m_Triangles.get_Item(num16 - 2);
					if (m_RemovedIndices.IsRemovedIndex(a_Index) || m_RemovedIndices.IsRemovedIndex(a_Index2) || m_RemovedIndices.IsRemovedIndex(a_Index3))
					{
						m_Triangles.RemoveRange(num16 - 2, 3);
					}
					else
					{
						int num17 = m_RemovedIndices.AdjustedIndex(a_Index);
						int num18 = m_RemovedIndices.AdjustedIndex(a_Index2);
						int num19 = m_RemovedIndices.AdjustedIndex(a_Index3);
						m_Triangles.set_Item(num16, num17);
						m_Triangles.set_Item(num16 - 1, num18);
						m_Triangles.set_Item(num16 - 2, num19);
					}
				}
		...
		public override void OffsetActiveProjectorVertices()
		{
			...
			if (!Mathf.Approximately(meshOffset, 0f))
			{
				...
				for (int i = decalsMeshLowerVertexIndex; i <= decalsMeshUpperVertexIndex; i++)
				{
					...
					m_Vertices.set_Item(i, m_Vertices.get_Item(i) + v);
				}
			}
		}
	}

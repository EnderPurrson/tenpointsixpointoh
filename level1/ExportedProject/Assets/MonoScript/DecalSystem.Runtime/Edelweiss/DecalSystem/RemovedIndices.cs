...
namespace Edelweiss.DecalSystem
{
	internal class RemovedIndices
	{
		...
		public int Count
		{
			get
			{
				return m_RemovedIndices.get_Count();
			}
		}
		...
		public void AddRemovedIndex(int a_Index)
		{
			if (a_Index < Count)
			{
				m_RemovedIndices.set_Item(a_Index, -1);
				m_GreatestUnremovedValue = -1;
				for (int i = a_Index + 1; i < Count; i++)
				{
					int num = m_RemovedIndices.get_Item(i) - 1;
					if (num >= 0)
					{
						m_GreatestUnremovedValue = num;
					}
					m_RemovedIndices.set_Item(i, num);
				}
			}
			else
			{
		...
		public bool IsRemovedIndex(int a_Index)
		{
			...
			return m_RemovedIndices.get_Item(a_Index) < 0;
		}
		...
		public int AdjustedIndex(int a_Index)
		{
			...
			return m_RemovedIndices.get_Item(a_Index);
		}
	}
}

...
namespace Edelweiss.DecalSystem
{
	public class SkinnedDecalsMeshCutter : GenericDecalsMeshCutter<SkinnedDecals, SkinnedDecalProjectorBase, SkinnedDecalsMesh>
	{
		...
		private int CutEdgeUnoptimized(int a_RelativeVertexLocationsOffset, int a_IndexA, int a_IndexB, Vector3 a_VertexA, Vector3 a_VertexB, bool a_IsVertexAInside, float a_IntersectionFactorAB)
		{
			...
			Vector3 a = m_DecalsMesh.OriginalVertices.get_Item(a_IndexA);
			Vector3 b = m_DecalsMesh.OriginalVertices.get_Item(a_IndexB);
			Vector3 a2 = m_DecalsMesh.Normals.get_Item(a_IndexA);
			Vector3 b2 = m_DecalsMesh.Normals.get_Item(a_IndexB);
			BoneWeight a_BoneWeight = m_DecalsMesh.BoneWeights.get_Item(a_IndexA);
			BoneWeight a_BoneWeight2 = m_DecalsMesh.BoneWeights.get_Item(a_IndexB);
			int count = m_DecalsMesh.Vertices.get_Count();
			...
			{
				m_DecalsMesh.UVs.Add(Vector2.Lerp(m_DecalsMesh.UVs.get_Item(a_IndexA), m_DecalsMesh.UVs.get_Item(a_IndexB), a_IntersectionFactorAB));
			}
			if (m_DecalsMesh.Decals.CurrentUV2Mode == UV2Mode.TargetUV || m_DecalsMesh.Decals.CurrentUV2Mode == UV2Mode.TargetUV2)
			{
				m_DecalsMesh.UV2s.Add(Vector2.Lerp(m_DecalsMesh.UV2s.get_Item(a_IndexA), m_DecalsMesh.UV2s.get_Item(a_IndexB), a_IntersectionFactorAB));
			}
			if (m_DecalsMesh.Decals.CurrentTangentsMode == TangentsMode.Target)
			{
				m_DecalsMesh.Tangents.Add(Vector4.Lerp(m_DecalsMesh.Tangents.get_Item(a_IndexA), m_DecalsMesh.Tangents.get_Item(a_IndexB), a_IntersectionFactorAB));
			}
			if (a_IsVertexAInside)
			{
				a_CutEdge.newVertex2Index = count;
				m_RelativeVertexLocations.set_Item(a_IndexB - a_RelativeVertexLocationsOffset, RelativeVertexLocation.Outside);
			}
			else
			{
				a_CutEdge.newVertex1Index = count;
				m_RelativeVertexLocations.set_Item(a_IndexA - a_RelativeVertexLocationsOffset, RelativeVertexLocation.Outside);
			}
		...
		private BoneWeight LerpBoneWeights(BoneWeight a_BoneWeight1, BoneWeight a_BoneWeight2, float a_Factor)
		{
			...
			global::System.Array.Sort<BoneWeightElement>(m_BoneWeightElements);

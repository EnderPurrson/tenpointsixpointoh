...
namespace Prime31
{
	public class P31RestKit
	{
		...
		protected virtual void processResponse(WWW www, Action<string, object> onComplete)
		{
			...
			{
				onComplete.Invoke(www.error, (object)null);
			}
			else if (isResponseJson(www))
			{
				...
				onComplete.Invoke((string)null, obj);
			}
			else
			{
				onComplete.Invoke((string)null, (object)www.text);
			}
		}
		...
		protected virtual IDictionary getHeadersFromForm(WWWForm form)
		{
			try
			{
				...
				if (property != null)
				{
					object value = property.GetValue((object)form, (object[])null);
